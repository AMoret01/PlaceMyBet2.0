...
namespace WebAPI.Models
{
    public class Repositorio_Apuestas
    {
        ...
        internal List<Apuesta> GetUsuario(double tipo_Mercado,string email)
        {
            Debug.WriteLine("Comienzo");
            MySqlConnection conectar = conexion();
            MySqlCommand command = conectar.CreateCommand();
            command.CommandText = "SELECT * FROM apuesta WHERE tipo_Mercado = @A and email = @A2";
            command.Parameters.AddWithValue("@A", tipo_Mercado);
        ...
        internal List<Apuesta> GetMercado(double tipo_Mercado, string email)
        {
            Debug.WriteLine("Comienzo");
            MySqlConnection conectar = conexion();
            MySqlCommand command = conectar.CreateCommand();
